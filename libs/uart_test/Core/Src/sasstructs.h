...
typedef struct
{
	...
	uint8_t	mainChnl;			//!< Nro de canal SAS principal
	uint8_t	titoChnl;			//!< Canal TITO
	uint8_t	aftChnl;			//!< Canal AFT
} cfgSAS;
 ...
 * Bills, los elementos del array representan los billetes:
...
typedef struct
{
	...
	uint8_t 	number[8];	//!< Validation number
...
typedef struct
{
	...
	uint16_t	poolId;				//!< Restricted pool ID
	uint8_t		cumCashAmntMt[9];	//!< Cummulative casheable amount meter, in cents BCD
	uint8_t		cumResAmntMt[9];	//!< Cummulative restricted amount meter, in cents BCD
	uint8_t		cumNonResAmntMt[9];	//!< Cummulative non restricted amount meter, in cents BCD
} aftTrx_t;
