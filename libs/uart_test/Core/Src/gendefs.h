...
#define POLL_ENADISRTE			((uint8_t)0x0E)
...
#define IHCASHTRTGM_MTR			0xA0 	//!< In-house cashable transfers to gaming machine (cents)
#define IHRESTRTGM_MTR 			0xA2 	//!< In-house restricted transfers to gaming machine (cents)
#define IHNONRESTRTGM_MTR 		0xA4 	//!< In-house nonrestricted transfers to gaming machine (cents)
#define DBTCASHTRHTOG_MTR		0xA6	//!< Debit transfers to gaming machine (cents)
#define IHCASHTRTOTKT_MTR		0xA8 	//!< In-house cashable transfers to ticket (cents)
#define IHRESTRTOTKT_MTR		0xAA	//!< In-house restricted transfers to ticket (cents)
#define DBTCASHTRHTOTKT_MTR		0xAC	//!< Debit transfers to ticket (cents)
#define BCASHTRHTG_MTR			0xAE 	//!< Bonus cashable transfers to gaming machine (cents)
#define IHCASHTRGTOH_MTR		0xB8	//!< In-house cashable transfers to host (cents)
#define IHRESTRGTOH_MTR			0xBA	//!< In-house restricted transfers to host (cents)
#define IHNRESTRGTOH_MTR		0xBC	//!< In-house nonrestricted transfers to host (cents)
#define BNRESTRHTG_MTR			0xB0	//!< Bonus nonrestricted transfers to gaming machine (cents)
...
/**
 * @brief Extended validation mask
 * 
 */
#define EXTVAL_PRNASCSHDEV	((uint16_t)0x0001)		//!< Use printer as cashout device
#define EXTVAL_PRNASHPDEV	((uint16_t)0x0002)		//!< Use printer as handpay receipt device
#define EXTVAL_VALHPANDHPR	((uint16_t)0x0004)		//!< Validate handpays and handpay receipts
#define EXTVAL_PRNRESTKT	((uint16_t)0x0008)		//!< Print restricted tickets
#define EXTVAL_TKTFFRESAMT	((uint16_t)0x0010)		//!< Tickets for foreign restricted amounts
#define EXTVAL_TKTREDEMPT	((uint16_t)0x0020)		//!< Ticket redemption
#define EXTVAL_SECENHVALC	((uint16_t)0x8000)		//!< Secure enhanced validation configuration
...
extern "C"
{
	...
	typedef int32_t(*poll_ptr)(const uint8_t*, const uint32_t, uint8_t*, uint32_t*);
