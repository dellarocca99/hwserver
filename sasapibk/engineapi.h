...
#define THREAD_SAFE_API     // Manejo thread safe   
...
namespace EngineAPI
{
    ...
    struct commData_t
    {
        ...
        uint8_t 	txBuff[MAX_TXRX];               //!< Buffer de Tx
        uint8_t 	rxBuff[MAX_TXRX];               //!< Buffer de Tx
        engFSMState decoState{engFSMState::esIdle}; //!< Estado de la FSM de recepcion
        command_t*  cmdRx;                          //!< Comando recibido desde el SE
    };
    ...
    struct configSE_t
    {
        ...
        bool        rteMode{false};                 //!< Real Time Events
    };
}
